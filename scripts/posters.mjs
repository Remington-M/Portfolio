#!/usr/bin/env node
/**
 * Draw the stills the site falls back to while a clip is still arriving.
 *
 * Two jobs, and they are different sizes. One still per PROJECT, for the deck
 * card, described below. And one per SHOT, which is the more important half:
 * a shot's <video> carries no poster unless it is given one, so until its
 * clip has buffered a frame the viewer renders an immaculate empty device —
 * correct frame, correct caption, correct tick marks, nothing inside. That
 * does not read as loading. It reads as though the work is a blank screen.
 *
 * Only the front card and its two neighbours are given a real <video> — six
 * autoplaying clips will not hold frame rate on a phone — so the rest of the
 * stack has nothing to show. Without a still they fall back to the flat empty
 * colour, which is dark: a card two places back rendered darker than the one
 * behind it, and the stack stopped reading as ordered by depth at all.
 *
 * The still is set on the <video> as well, so a live card shows the picture
 * from the first frame rather than the empty colour while it decodes.
 *
 * JPEG rather than WebP, which would be about a third the size: the ffmpeg on
 * this machine is built without libwebp, and a poster is not worth making the
 * toolchain a prerequisite over. At this size the difference is a few tens of
 * kilobytes across the whole set.
 *
 *   node scripts/posters.mjs
 */
import { execFile } from "node:child_process";
import { readFile, writeFile, stat } from "node:fs/promises";
import { dirname, join, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { promisify } from "node:util";

const run = promisify(execFile);
const REPO = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const source = await readFile(join(REPO, "lib/projects.ts"), "utf8");

/**
 * Wide enough for the card at 2x and no wider.
 *
 * The deck card is around 400 points across, and this is a still nobody looks
 * at closely — it is what a card two places back is wearing.
 */
const WIDTH = 800;
/**
 * Shot stills are smaller than the viewer that shows them.
 *
 * A poster is on screen for the few hundred milliseconds before the clip has a
 * frame, and every byte of it is a byte the clip is not getting. Sharp enough
 * that type is legible, and no sharper — the whole point is to be replaced.
 */
const SHOT_WIDTH = 720;
const SHOT_QUALITY = 5;
/**
 * The first frame, and it has to be the first frame.
 *
 * A poster is what the card wears until it plays, and these clips play from
 * the start — so anything else is a picture that jumps the instant playback
 * begins. Taken from the middle, as this first did, every card in the stack
 * showed a moment the clip has not reached yet and then cut backwards to its
 * opening the moment it came to the front.
 */
const AT = 0;

const marks = [...source.matchAll(/slug: "([a-z0-9-]+)",/g)].map((m) => ({
  slug: m[1],
  at: m.index,
}));

let wrote = 0;
for (const [i, mark] of marks.entries()) {
  const end = i + 1 < marks.length ? marks[i + 1].at : source.length;
  const text = source.slice(mark.at, end);
  const head = text.slice(0, text.indexOf("shots:"));
  const hero = head.match(/src: "(\/media\/[^"]+)"/)?.[1];
  if (!hero) {
    console.log(`  ${mark.slug.padEnd(24)} no hero clip, skipped`);
    continue;
  }

  const clip = join(REPO, "public", hero);
  /**
   * Beside the clip it was taken from, not in a folder named for the project.
   *
   * The listing editor's media still lives under `mys`, so a path built from
   * the slug pointed at a directory that does not exist. Following the clip
   * keeps a project's media in one place whatever that place is called.
   */
  const out = `${hero.slice(0, hero.lastIndexOf("/"))}/poster.jpg`;
  const outFile = join(REPO, "public", out);

  const at = AT;

  await run("ffmpeg", [
    "-hide_banner", "-loglevel", "error", "-y",
    "-ss", String(at), "-i", clip, "-frames:v", "1",
    "-vf", `scale=${WIDTH}:-2`,
    // -q:v 3 is near the top of mjpeg's quality range. These are flat UI
    // screens, where ringing around type shows up far more readily than it
    // would on a photograph.
    "-c:v", "mjpeg", "-q:v", "4",
    outFile,
  ]);

  const size = (await stat(outFile)).size;
  console.log(
    `  ${mark.slug.padEnd(24)} ${out}  ${(size / 1024).toFixed(0)}KB  ` +
      `from ${hero.replace("/media/", "")} @ ${at.toFixed(2)}s`,
  );
  wrote += 1;
}

/* ------------------------------------------------------------------ *
 * Shots
 *
 * Every clip referenced by a shot, across every project, gets a still beside
 * it: `about-you-editor.mp4` becomes `about-you-editor.jpg`. Named after the
 * clip rather than the shot so the pairing survives a shot being renamed,
 * reordered, or shared between two projects — the gesture clip is in two.
 * ------------------------------------------------------------------ */
const shotClips = [
  ...new Set([...source.matchAll(/src: "(\/media\/[^"]+\.mp4)"/g)].map((m) => m[1])),
].sort();

const withPoster = [];
for (const clip of shotClips) {
  const out = clip.replace(/\.mp4$/, ".jpg");
  const outFile = join(REPO, "public", out);
  try {
    await run("ffmpeg", [
      "-hide_banner", "-loglevel", "error", "-y",
      "-ss", String(AT), "-i", join(REPO, "public", clip), "-frames:v", "1",
      "-vf", `scale=${SHOT_WIDTH}:-2`,
      "-c:v", "mjpeg", "-q:v", String(SHOT_QUALITY),
      outFile,
    ]);
  } catch (err) {
    console.log(`  ! ${clip} — ${err.message.split("\n")[0]}`);
    continue;
  }
  withPoster.push(clip);
  const size = (await stat(outFile)).size;
  console.log(`  ${out.replace("/media/", "").padEnd(52)} ${(size / 1024).toFixed(0)}KB`);
}

/**
 * The manifest, so the site never points at a still that was not drawn.
 *
 * A poster path could be derived from the clip path in the app with one
 * `replace` and no file at all — but a derived path for a still that does not
 * exist is a 404 on every shot, and the export is static so nothing can check
 * at runtime. Writing the list the script actually produced keeps the two in
 * step: no entry, no poster attribute, exactly the behaviour there was before.
 */
const manifest =
  `/**\n` +
  ` * Generated by \`npm run posters\`. Do not edit.\n` +
  ` *\n` +
  ` * Clips that have a still beside them, as \`<name>.jpg\`. See the note in\n` +
  ` * \`scripts/posters.mjs\` for why this is a list rather than a derived path.\n` +
  ` */\n` +
  `export const CLIPS_WITH_POSTER = new Set<string>([\n` +
  withPoster.map((c) => `  ${JSON.stringify(c)},`).join("\n") +
  (withPoster.length ? "\n" : "") +
  `]);\n`;
await writeFile(join(REPO, "lib/posters.generated.ts"), manifest);

console.log(`\n${wrote} project posters, ${withPoster.length} shot posters written.`);
